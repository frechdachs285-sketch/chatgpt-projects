...
class MeasurementsValidator {
  List<String> validate(Measurements m) {
    ...
    if (m.hipDepth >= m.skirtLength) {
      errors.add('Die Rocklaenge muss groesser als die Huefttiefe sein.');
    }
    return errors;
  }
}
...
class SkirtPatternCalculator {
  ...
  Map<String, PatternPoint> _calculatePoints(Measurements m, ConstructionValues c) {
    ...
    final p = <String, PatternPoint>{
      ...
      'P2': PatternPoint(width, 0),
      'P3': PatternPoint(0, m.skirtLength),
      'P4': PatternPoint(width, m.skirtLength),
      'P5': PatternPoint(0, m.hipDepth),
      'P6': PatternPoint(width, m.hipDepth),
      'P7': PatternPoint(sideX, m.hipDepth),
      'P8': PatternPoint(sideX, m.skirtLength),
      'P9': PatternPoint(backWaistX, 0),
      'P10': PatternPoint(backWaistX, -c.sideWaistLift),
      'P15': PatternPoint(frontWaistX, 0),
      'P16': PatternPoint(frontWaistX, -c.sideWaistLift),
    };

    final backVector = p['P10']! - p['P1']!;
    p['P11'] = p['P1']! + backVector * (1 / 3);
    p['P12'] = p['P1']! + backVector * (2 / 3);
    p['P13'] = _dartApex(p['P11']!, backVector, c.backDart1Length);
    p['P14'] = _dartApex(p['P12']!, backVector, c.backDart2Length);

    final frontVector = p['P16']! - p['P2']!;
    p['P17'] = p['P2']! + frontVector * (1 / 3);
    p['P18'] = _dartApex(p['P17']!, frontVector, c.frontDartLength);
