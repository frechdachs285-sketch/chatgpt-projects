...
class _TrouserPageState extends State<TrouserPage> {
  ...
  int _selectedSizeCode = 14;
  int _appliedSizeCode = 14;
  ...
  TrouserMeasurements? get _measurements {
    final values = [
      _read(_waistController),
      _read(_hipController),
      _read(_hipDepthController),
      _read(_bodyRiseController),
      _read(_waistToFloorController),
      _read(_bottomWidthController),
    ];
    ...
    return TrouserMeasurements(
      ...
      hipDepth: values[2]!,
      bodyRise: values[3]!,
      waistToFloor: values[4]!,
      trouserBottomWidth: values[5]!,
    );
  }
  ...
  TrouserSeamAllowanceSettings? get _seamAllowance {
    ...
    final s = TrouserSeamAllowanceSettings(
  ...
  void _calculate() {
    ...
    try {
      ...
      final front = pieceBuilder.front(
        draft,
        seamAllowance: seamAllowance,
        sizeCode: _selectedSizeCode,
      );
      final back = pieceBuilder.back(
        draft,
        seamAllowance: seamAllowance,
        sizeCode: _selectedSizeCode,
      );
      ...
      setState(() {
        ...
        _appliedSizeCode = _selectedSizeCode;
        _message = null;
      });
    } on ArgumentError catch (e) {
      setState(() => _message = e.message?.toString() ?? 'Maße bitte prüfen.');
  ...
  Future<void> _openPatternPdf() async {
    final m = _appliedMeasurements;
    if (m == null) return;
    if (_appliedSizeCode != 14) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'PDF für andere Konstruktionsgrößen wird im nächsten Schritt angebunden. Größe 14 bleibt vollständig verfügbar.',
          ),
        ),
      );
      return;
  ...
  Widget _seamField(String label, TextEditingController c) => Padding(
        padding: const EdgeInsets.only(bottom: 10),
        child: TextField(
          controller: c,
          enabled: _seamAllowanceEnabled,
          ...
          children: [
            ...
            DropdownButtonFormField<int>(
              ...
              decoration: const InputDecoration(
                labelText: 'Aldrich-Konstruktionsgröße',
                border: OutlineInputBorder(),
                isDense: true,
                helperText: 'Wird ausdrücklich gewählt und nicht aus Körpermaßen geschätzt.',
              ),
              items: [
                for (final size in TrouserSizeRules.supportedSizeCodes)
                  DropdownMenuItem(value: size, child: Text('Größe $size')),
              ],
              onChanged: (value) {
                ...
                child: Column(
                  children: [
                    SwitchListTile(
                      ...
                      onChanged: (v) {
                        setState(() => _seamAllowanceEnabled = v);
                        _calculate();
                      },
                    ),
                    _seamField('Seitennähte, Innenbein und Schritt', _normalSeamController),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Konstruktionsgröße: $_appliedSizeCode'),
                      ...
                      Text('Bund: ${waistband.outline.segments.length} Kontursegmente, ${waistband.guideLines.length} Markierungs-/Bruchlinien'),
                      Text(_appliedSeamAllowance?.enabled == true ? 'Nahtzugabe: an' : 'Nahtzugabe: aus'),
                    ],
                  ),
                ),
              ),
