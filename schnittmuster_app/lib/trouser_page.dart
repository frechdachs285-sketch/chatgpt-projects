...
class _TrouserPageState extends State<TrouserPage> {
  ...
  @override
  void dispose() {
    _waistController.dispose(); _hipController.dispose(); _hipDepthController.dispose();
    _bodyRiseController.dispose(); _waistToFloorController.dispose(); _bottomWidthController.dispose();
    _normalSeamController.dispose(); _waistSeamController.dispose(); _hemSeamController.dispose();
    super.dispose();
  }
  ...
  double? _readNonNegative(TextEditingController c) {
    final v = double.tryParse(c.text.trim().replaceAll(',', '.'));
    return v == null || !v.isFinite || v < 0 ? null : v;
  }

  TrouserMeasurements? get _measurements {
    final values = [_read(_waistController), _read(_hipController), _read(_hipDepthController), _read(_bodyRiseController), _read(_waistToFloorController), _read(_bottomWidthController)];
    if (values.any((v) => v == null)) return null;
    return TrouserMeasurements(waist: values[0]!, hip: values[1]!, hipDepth: values[2]!, bodyRise: values[3]!, waistToFloor: values[4]!, trouserBottomWidth: values[5]!);
  }

  TrouserSeamAllowanceSettings? get _seamAllowance {
    final normal = _readNonNegative(_normalSeamController), waist = _readNonNegative(_waistSeamController), hem = _readNonNegative(_hemSeamController);
    if (normal == null || waist == null || hem == null) return null;
    final s = TrouserSeamAllowanceSettings(enabled: _seamAllowanceEnabled, normalCm: normal, waistCm: waist, hemCm: hem);
    return s.isValid ? s : null;
  }

  void _calculate() {
    FocusScope.of(context).unfocus();
    final measurements = _measurements, seamAllowance = _seamAllowance;
    ...
    try {
      ...
      const pieceBuilder = TrouserPatternPieceBuilder(), waistbandBuilder = TrouserWaistbandBuilder();
      ...
      setState(() { _draft = draft; _front = front; _back = back; _waistband = waistband; _appliedMeasurements = measurements; _appliedSeamAllowance = seamAllowance; _message = null; });
    } on ArgumentError catch (e) { setState(() => _message = e.message?.toString() ?? 'Maße bitte prüfen.'); }
      on StateError catch (e) { setState(() => _message = e.message); }
  }

  Future<void> _openPatternPdf() async {
    final m = _appliedMeasurements; if (m == null) return;
    try {
      final bytes = await TrouserPdfExporter().buildPatternPdf(measurements: m, seamAllowance: _appliedSeamAllowance);
      await Printing.layoutPdf(name: 'Hose_v1_Schnittmuster_1zu1.pdf', onLayout: (_) async => bytes);
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Hose-PDF konnte nicht erstellt werden. Bitte Maße prüfen.')));
    }
  }
  ...
  Widget build(BuildContext context) {
    final draft = _draft, front = _front, back = _back, waistband = _waistband;
    return Scaffold(
      ...
      body: SafeArea(child: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        _field('Taillenumfang', _waistController), _field('Hüftumfang', _hipController), _field('Hüfttiefe', _hipDepthController), _field('Sitzhöhe', _bodyRiseController), _field('Taille bis Boden', _waistToFloorController), _field('Fertige Saumweite je Hosenbein', _bottomWidthController),
        const SizedBox(height: 8), Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [
          SwitchListTile(contentPadding: EdgeInsets.zero, title: const Text('Nahtzugabe'), value: _seamAllowanceEnabled, onChanged: (v) { setState(() => _seamAllowanceEnabled = v); _calculate(); }),
          _seamField('Seitennähte, Innenbein und Schritt', _normalSeamController), _seamField('Taille', _waistSeamController), _seamField('Saum', _hemSeamController),
        ]))),
        ...
        if (draft != null && front != null && back != null && waistband != null) ...[
          const SizedBox(height: 16), Card(child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Berechnung erfolgreich'), const SizedBox(height: 6), Text('Referenzpunkte: ${draft.points.length}'),
            Text('Vorderhose: ${front.outline.segments.length} Kontursegmente, ${front.darts.length} Abnäher'), Text('Hinterhose: ${back.outline.segments.length} Kontursegmente, ${back.darts.length} Abnäher'), Text('Bund: ${waistband.outline.segments.length} Kontursegmente, ${waistband.guideLines.length} Markierungs-/Bruchlinien'), Text(_appliedSeamAllowance?.enabled == true ? 'Nahtzugabe: an' : 'Nahtzugabe: aus'),
          ]))),
          const SizedBox(height: 16), Text('Vorschau', style: Theme.of(context).textTheme.titleLarge), const SizedBox(height: 8),
          TrouserPreview(front: front, back: back, waistband: waistband),
          const SizedBox(height: 16), SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: _openPatternPdf, icon: const Icon(Icons.picture_as_pdf_outlined), label: const Text('PDF 1:1 öffnen'))),
        ],
      ])),
    );
  }
