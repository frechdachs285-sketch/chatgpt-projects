...
class _TrouserPageState extends State<TrouserPage> {
  ...
  void _calculate() {
    ...
    try {
      ...
      setState(() { _draft = draft; _front = front; _back = back; _waistband = waistband; _appliedMeasurements = measurements; _appliedSeamAllowance = seamAllowance; _appliedSizeCode = _selectedSizeCode; _message = null; });
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Hose v1')), body: SafeArea(child: ListView(padding: const EdgeInsets.all(16), children: [
      ...
      if (draft != null && front != null && back != null && waistband != null) ...[const SizedBox(height: 16), Card(child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Berechnung erfolgreich'), const SizedBox(height: 6), Text('Konstruktionsgröße: $_appliedSizeCode'), Text('Referenzpunkte: ${draft.points.length}'), Text('Vorderhose: ${front.outline.segments.length} Kontursegmente, ${front.darts.length} Abnäher'), Text('Hinterhose: ${back.outline.segments.length} Kontursegmente, ${back.darts.length} Abnäher'), Text('Bund: ${waistband.outline.segments.length} Kontursegmente, ${waistband.guideLines.length} Markierungs-/Bruchlinien'), Text(_appliedSeamAllowance?.enabled == true ? 'Nahtzugabe: an' : 'Nahtzugabe: aus')]))), const SizedBox(height: 16), Text('Vorschau', style: Theme.of(context).textTheme.titleLarge), const SizedBox(height: 8), TrouserPreview(front: front, back: back, waistband: waistband), const SizedBox(height: 16), SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: _openPatternPdf, icon: const Icon(Icons.picture_as_pdf_outlined), label: const Text('PDF 1:1 öffnen')))],
    ])));
  }
}
