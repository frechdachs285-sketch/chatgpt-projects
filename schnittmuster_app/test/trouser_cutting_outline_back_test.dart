...
import 'package:schnittmuster_app/trouser_cutting_outline_back.dart';
...
void main() {
  ...
  test('buildBackCuttingOutline returns a closed continuous polyline', () {
    final draft = TrouserPatternCalculator.calculateReferencePoints(measurements);
    final seamOutline = outlineBuilder.backLowerContour(draft);
    final cuttingOutline = cuttingBuilder.buildBackCuttingOutline(
      outline: seamOutline,
      draft: draft,
      settings: settings,
    );
  ...
  test('buildBackCuttingOutline does not modify the confirmed seam outline', () {
    final draft = TrouserPatternCalculator.calculateReferencePoints(measurements);
    final seamOutline = outlineBuilder.backLowerContour(draft);
    final originalSegments = List<PathSegment>.from(seamOutline.segments);

    cuttingBuilder.buildBackCuttingOutline(
      outline: seamOutline,
      draft: draft,
      settings: settings,
    );
  ...
  test('buildBackCuttingOutline rejects disabled seam allowance', () {
    final draft = TrouserPatternCalculator.calculateReferencePoints(measurements);
    final seamOutline = outlineBuilder.backLowerContour(draft);
