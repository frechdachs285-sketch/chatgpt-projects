  ...
  Future<void> _chooseRight(String right) async {
    ...
    if (_puzzle.pairs[left] == right) {
      ...
      if (_solved.length == _puzzle.pairs.length) {
        await _speech.speak('Juhuuu! Alles richtig zugeordnet. Tagesrätsel geschafft!');
      } else {
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            children: [
              RaetseliMascot(
                message: finished ? 'Super! 🎉 Du hast alle Paare gefunden!' : '${_puzzle.title} Tippe erst links und dann rechts. 👆',
                mascotSize: 72,
                mascotEmojiSize: 43,
                messageFontSize: 15,
              ),
              ...
              Expanded(
                child: Row(
                  children: [
                    ...
                    Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: rightItems.map((item) => _rightButton(item)).toList())),
                  ],
                ),
              ),
              Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFFFFF0B8), borderRadius: BorderRadius.circular(20)),
                child: Text(
                  finished ? '🎉 Tagesrätsel geschafft!' : _selectedLeft == null ? 'Wähle links ein Bild aus' : 'Jetzt finde rechts das passende Bild',
              ...
              if (finished) ...[
                ...
                SizedBox(width: double.infinity, height: 54, child: FilledButton(onPressed: () => Navigator.pop(context), child: const Text('Zurück zu RätselKids ⭐', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)))),
              ],
            ],
          ),
        ),
  ...
  Widget _leftButton(String item) {
    ...
    return SizedBox(
      ...
      child: FilledButton.tonal(
        onPressed: solved ? null : () { HapticFeedback.lightImpact(); setState(() => _selectedLeft = item); },
        style: FilledButton.styleFrom(
          backgroundColor: selected ? const Color(0xFFD8D4FF) : solved ? const Color(0xFFDDF5E6) : Colors.white,
          disabledBackgroundColor: const Color(0xFFDDF5E6),
          disabledForegroundColor: const Color(0xFF2B2B3A),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        ),
        child: Text(solved ? '$item ✓' : item, textAlign: TextAlign.center, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
      ),
    );
  }
