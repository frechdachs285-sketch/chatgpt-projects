...
class _DailyPuzzleScreenState extends State<DailyPuzzleScreen> {
  ...
  void initState() {
    ...
    if (!_showMatching) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _speech.speak(
          'Tagesrätsel! Tippe die Zahlen der Reihe nach an. Beginne mit der Eins.',
        );
      });
    }
  }
  ...
  Future<void> _tapPoint(int number) async {
    ...
    if (number == _motif.points.length) {
      ...
      if (mounted) {
        setState(() => _earnedStar = earned);
      }

      await _speech.speak(
        earned
            ? 'Juhuuu! Du hast alle Zahlen verbunden. Es ist ein ${_motif.name}! Du bekommst deinen Tagesstern!'
            : 'Juhuuu! Es ist ein ${_motif.name}! Das Tagesrätsel hast du heute schon geschafft!',
      );
    } else {
  ...
  Widget build(BuildContext context) {
    if (_showMatching) {
      return const DailyMatchingScreen();
    }
          ...
          child: Column(
            children: [
              RaetseliMascot(
                message: _finished
                    ? (_earnedStar
                        ? 'Geschafft! ${_motif.emoji} Dein Tagesstern ist da! ⭐'
                        : 'Geschafft! ${_motif.emoji} Heute schon erledigt!')
                            ...
                            Positioned.fill(
                              child: CustomPaint(
                                painter: _ConnectPainter(
                                  points: _motif.points,
                                  connectedCount: _nextNumber - 1,
                                  finished: _finished,
                                ),
                              ),
                            ),
...
class _ConnectPainter extends CustomPainter {
  ...
  bool shouldRepaint(covariant _ConnectPainter oldDelegate) {
    return oldDelegate.connectedCount != connectedCount ||
