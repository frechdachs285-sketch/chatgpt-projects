...
class _DailyPuzzleScreenState extends State<DailyPuzzleScreen> {
  ...
  Future<void> _tapPoint(int number) async {
    if (_finished || number != _nextNumber) {
      if (!_finished) HapticFeedback.selectionClick();
      return;
    }
          ...
          child: Column(
            children: [
                        ...
                        child: Stack(
                          children: [
                            ...
                            ..._points.asMap().entries.map((entry) {
                              ...
                              final done = number < _nextNumber;
                              return Positioned(
                                left: point.dx * size - 23,
                                top: point.dy * size - 23,
                                child: GestureDetector(
                                  onTap: () => _tapPoint(number),
                                  child: AnimatedContainer(
                                    ...
                                    decoration: BoxDecoration(
                                      color: done ? const Color(0xFFFFD966) : Colors.white,
                                      shape: BoxShape.circle,
                                      border: Border.all(color: done ? const Color(0xFFE0A900) : const Color(0xFF6E68A8), width: 3),
                                      boxShadow: const [BoxShadow(blurRadius: 7, offset: Offset(0, 3), color: Color(0x22000000))],
                                    ),
                                    child: Text('$number', style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900, color: Color(0xFF302E48))),
                                  ),
                                ),
                              );
                            }),
              ...
              Container(
                ...
                child: Text(
                  _finished ? '🎉 Tagesrätsel geschafft!' : 'Als Nächstes: $_nextNumber',
              ...
              if (_finished) ...[
                ...
                SizedBox(
                  ...
                  child: FilledButton(
                    ...
                    child: const Text('Zurück zu RätselKids ⭐', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
                  ),
                ),
              ],
            ],
...
class _ConnectPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 1; i < connectedCount && i < points.length; i++) {
      path.lineTo(points[i].dx * size.width, points[i].dy * size.height);
    }
    if (finished) {
      path.lineTo(first.dx * size.width, first.dy * size.height);
    }
