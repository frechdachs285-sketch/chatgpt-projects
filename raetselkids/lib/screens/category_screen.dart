...
class _CategoryScreenState extends State<CategoryScreen> {
  ...
  Widget _modeButton(BuildContext context, String emoji, String title, String subtitle, Color color, RoundMode value) {
    return SizedBox(
      ...
      child: FilledButton(
        ...
        child: Row(children: [
          ...
          Text(subtitle, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800)),
        ]),
      ),
    );
  }
  ...
  Widget _categoryCard({required String id, required String title, required String emoji, required Color color, required List<Puzzle> puzzles}) {
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: _cardBody(
            emoji: emoji,
            title: title,
            subtitle: best == 0 ? '${puzzles.length} Rätsel' : '⭐ $best / ${puzzles.length}',
            complete: complete,
          ),
        ),
      ),
    ...
    return Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(30),
          onTap: () async {
            await Navigator.push(context, MaterialPageRoute(builder: (_) => const EnglishCategoryScreen()));
            await _load();
          },
          child: _cardBody(
            emoji: '🇬🇧',
            title: 'Englisch',
            subtitle: '33 Rätsel · 4 Bereiche',
            complete: false,
          ),
        ),
      ),
  ...
  Widget _cardBody({required String emoji, required String title, required String subtitle, required bool complete}) {
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(title, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Color(0xFF302E48))),
              const SizedBox(height: 5),
              Text(subtitle, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Color(0xFF514F61))),
            ],
          ),
        ),
        Container(
          ...
          alignment: Alignment.center,
          decoration: BoxDecoration(color: Colors.white.withValues(alpha: .72), shape: BoxShape.circle),
          child: Text(complete ? '✓' : '›', style: const TextStyle(fontSize: 27, fontWeight: FontWeight.w900, color: Color(0xFF514F61))),
        ),
      ]),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Stack(
          children: [
            const Positioned(top: 25, right: 18, child: Opacity(opacity: .5, child: Text('⭐', style: TextStyle(fontSize: 24)))),
            const Positioned(top: 95, left: 12, child: Opacity(opacity: .35, child: Text('🧩', style: TextStyle(fontSize: 28)))),
            ListView(
                ...
                Container(
                  ...
                  child: const Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 10),
                      Flexible(child: Text('Such dir eine Welt aus!', textAlign: TextAlign.center, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Color(0xFF3D3A58)))),
                    ],
                  ),
                ),
